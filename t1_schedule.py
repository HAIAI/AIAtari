...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        """
        if self.nsteps > 0:
            t = min(max(0, t), self.nsteps)
            step_i = (self.eps_end - self.eps_begin) / self.nsteps
            self.epsilon = self.eps_begin + step_i * t
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        N.B:  with probability self.epsilon, return a random action
              else, return best_action

              access the environment via self.env

              use env.action_space.sample() to generate a random action        
        """
        p = np.random.rand()
        if p <= self.epsilon:
...
def extra_test():
    """
    Use this to implement extra tests if needed
    """
    pass
